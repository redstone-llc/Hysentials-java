        ...
        suspend fun run() {
            ...
            val latestRelease = when (HysentialsConfig.updateChannel) {
                1 -> let {
                    ...
                    body.type = "beta"
                    body.downloadUrl =
                        "https://backend.redstone.llc/api/update/file?type=beta&uuid=${Minecraft.getMinecraft().session.profile.id.toString()}&key=${Socket.serverId}"
                    body
                }
                ...
                2 -> {
                    let {
                        ...
                        body.type = "dev"
                        body.downloadUrl =
                            "https://backend.redstone.llc/api/update/file?type=dev&uuid=${Minecraft.getMinecraft().session.profile.id.toString()}&key=${Socket.serverId}"
                        body
                    }
                }
