...
object RendererImpl : Renderer {
    @JvmStatic
    private val LOGGER = LogManager.getLogger("Hysentials/Renderer")
    ...
    private val PIXELS: ByteBuffer = run {
        ...
        (MemoryUtil.memAlloc(arr.size).put(arr) as Buffer).flip() as ByteBuffer
    }
    ...
    override fun init() {
        ...
        val fit = NVGFont(nvgCreateFontMem(vg, font.name, fdata, 0), fdata)
    ...
    override fun dropShadow(
        ...
        nvgRoundedRect(
            ...
            x - spread,
            y - spread - blur,
            width + spread * 2f + blur * 2f,
            height + spread * 2f + blur * 2f,
            radius + spread
        )
    ...
    private fun getFontSync(font: Font): Int {
        return fonts.getOrPut(font) {
            ...
            NVGFont(nvgCreateFontMem(vg, font.name, data, 0), data)
        }.id
    }
    ...
    private fun svgLoad(image: PolyImage, data: ByteBuffer): Int {
        val svg =
            nsvgParse(data, PIXELS, 96f) ?: throw IllegalStateException("Failed to parse SVG: ${image.resourcePath}")
