package cc.woverflow.hysentials.guis.hsplayerlist;
...
import static cc.woverflow.hysentials.schema.HysentialsSchema.*;
import static cc.woverflow.hysentials.util.BlockWAPIUtils.getDisplayName;

@SideOnly(Side.CLIENT)
public class GuiOnlineList extends Gui {
    private static final Ordering<User> field_175252_a = Ordering.from(new PlayerComparator());
    ...
    public void renderPlayerlist(int width) {
        ...
        List<User> list = field_175252_a.sortedCopy(Socket.cachedUsersNew.values());
        int i = 0;
        int j = 0;
        ...
        while(iterator.hasNext()) {
            ...
            k = this.mc.fontRendererObj.getStringWidth(getDisplayName(user));
            i = Math.max(i, k);
        }
        ...
        for(int t = 0; t < l; ++t) {
            ...
            if (t < list.size()) {
                User user = list.get(t);
                String string2 = getDisplayName(user);
                if (bl) {
    ...
    @SideOnly(Side.CLIENT)
    static class PlayerComparator implements Comparator<User> {
