...
public class HysentialsMods extends Page {

    private final ArrayList<HysentialsModCard> modCards = new ArrayList<>();
    private final ArrayList<BasicButton> modCategories = new ArrayList<>();
    private int size;

    public HysentialsMods() {
        super("Hysentials Mods");
        reloadMods();
        ...
        HysentialsConfig config = Hysentials.INSTANCE.getConfig();
        modCards.add(new HysentialsModCard(config.chatConfig.mod, true, false, false, this));
        modCards.add(new HysentialsModCard(config.formattingConfig.mod, true, false, false, this));
        modCards.add(new HysentialsModCard(config.housingConfig.mod, true, false, false, this));
        modCards.add(new HysentialsModCard(config.mod, true, false, false, this));
    ...
    public void draw(long vg, int x, int y, InputHandler inputHandler) {
        ...
        int iX = x + 16;
        int iY = y + 72;
        ArrayList<HysentialsModCard> finalModCards = new ArrayList<>(modCards);
        for (HysentialsModCard modCard : finalModCards) {
        ...
        size = iY - y + 135;
        if (iX == x + 16 && iY == y + 72) {
            NanoVGHelper.INSTANCE.drawText(vg, "Looks like there is nothing here. Try another category?", x + 16, y + 72, new Color(255, 255, 255, 153).getRGB(), 14f, Fonts.MEDIUM);
        }
    }
    ...
    private boolean inSelection(HysentialsModCard modCard) {
