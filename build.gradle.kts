...
loom {
    ...
    if (project.platform.isForge) {
        forge {
            ...
            accessTransformer(rootProject.file("src/main/resources/hysentials_at.cfg"))
        }
    }
...
tasks {
    ...
    processResources {
        ...
        filesMatching(listOf("mcmod.info", "mixins.hysentials.json", "mods.toml")) {
            expand(
                mapOf(
                    "id" to mod_id,
                    "name" to mod_name,
    ...
    jar {
        ...
        if (platform.isLegacyForge) {
            manifest.attributes += mapOf(
                ...
                "MixinConfigs" to "mixins.hysentials.json", // We want to use our mixin configuration, so we specify it here.
